...
def config():
	...
	training = dict(
		opt='Adam',
		learning_rate=0.001,
		training_epochs=50000,#50000
		batch_size= 1) # round(sum(class_['size'] for class_ in dataset['classes'])/100))
	learning_model = dict(
		model_type='Mlp',
		layers_sizes=[1, 1],
		activation='linear',
...
def run(_run, training, learning_model):
	...
	batch_size = round(len(x_train)*1.0)
	#print('batch size',batch_size==len(x_train))
	...
	for epoch in range(1, training_epochs + 1):
		...
		# save loss in training history for each epoch
		training_history['loss'].append(loss.numpy().mean())

			# updating gradients history
			#for idx in range(len(model.layers)):
			#	gradients_dict[f'layer_{idx}']['weights'].append(gradients[2*idx].numpy())
			#	gradients_dict[f'layer_{idx}']['bias'].append(gradients[2*idx+1].numpy())

		# updating weights history after each epoch
		for idx in range(len(model.layers)):
			weights_dict[f'layer_{idx}']['weights'].append(weights[2*idx].numpy())
			weights_dict[f'layer_{idx}']['bias'].append(weights[2*idx+1].numpy())
	...
	dir_name = "id_" + str(get_run_id()) + "_" + datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S")
	dir_path = os.path.join(os.path.abspath('../output'), "artifacts", dir_name)
	...
	train_history_path = os.path.join(dir_path, 'training_history.pkl')
	weights_history_path = os.path.join(dir_path, 'weights_history.pkl')
	#gradients_history_path = os.path.join(dir_path, 'gradients_history.pkl')
	...
	t2 = time.time()
	#print(f"Artifacts saved in {t2 - t1} seconds")
